<template>
  <div>
    <v-row justify="center">
      <v-col>
           <h2 class="text-center text-primary">Résultats OGOOUE MARITIME</h2>
        <!-- <line-chart :data="dataTest" /> -->
        <column-chart :data="dataTest" :colors="pieChartColors"></column-chart>
      </v-col>
    </v-row>
  </div>
...
export default {
  ...
  methods: {
    async getData() {
      await this.$axios
        .get("/voting_centre/by_province/08")
        .then((response) => {
