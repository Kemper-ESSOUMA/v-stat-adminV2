...
export default {
  ...
  methods: {
    ...
    connectWebSocket() {
      ...
      this.ws.onmessage = (event) => {
        ...
        try {
          ...
          // Si le message contient des données mises à jour
          if (message && message.updatedData) {
            // Mettre à jour les données
            this.datas = [...this.datas, ...message.updatedData]; // Ajoutez les nouvelles données

            // Redessiner les graphiques avec les nouvelles données
            this.renderSecondChart();
            this.renderChartProvince(); 
          }

          if (message && message.updatedProvinces) {
            // Mettre à jour les données des provinces
            this.province = message.updatedProvinces;

            // Redessiner le graphique des provinces
            this.renderChartProvince(); // Redessiner le graphique des provinces
          }
        } catch (error) {
          console.error(
    ...
    renderChartProvince() {
      ...
      console.log("datas=", this.province)
