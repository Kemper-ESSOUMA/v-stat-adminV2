<template>
  ...
  <div
    class="card d-flex flex-column align-items-center justify-content-between"
  >
    ...
    <!-- Troisieme graphique avec son titre -->
    <!-- <div class="chart-container" style="width: 90%; margin-bottom: 10px">
      <h5 class="chart-title text-center">Votes par siege</h5>
      <div
        class="button-container text-center"
        v-if="currentUser().role === 'SUPER_ADMIN'"
      >
      ...
      <canvas id="siege" width="400" height="200"></canvas>
    </div> -->
    ...
    <div class="chart-container" style="width: 90%; margin-bottom: 20px">
      ...
      >
        <button
          v-for="(zone, index) in zones"
          :key="index"
          @click="handleZoneAndSiege(zone.code)"
          :class="['data-btn', { active: selectedZone === zone.code }]"
        >
          {{ zone.name }}
        </button>
    ...
    <!-- Graphique Siège (en bas) -->
    <div
      class="chart-container"
      style="width: 90%; margin-bottom: 10px; margin-top: 20px"
    >
      <h5 class="chart-title text-center">Votes par Siège</h5>
      <canvas id="siege" width="400" height="200"></canvas>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    get_stat_by_siege() {
      this.$axios
        .get("/siege/stat_by_zone", { params: { zone: this.selectedZone } })
        .then((response) => {
          this.datas = response.data;
          this.renderget_stat_by_siege();
        })
        .catch((error) =>
          console.error("Erreur de récupération des statistiques :", error)
        );
    ...
    renderChartProvince() {
      ...
      const candidates = Object.keys(this.province[0]).filter((key) =>
      ...
      const labels = candidates.map(
        (candidate) => this.province[0][candidate]?.name || `${candidate}`
      );
      const values = this.province.map(
        (item) => candidates.map((candidate) => item[candidate]?.data || 0) // Valeur par défaut 0 si undefined
      );
