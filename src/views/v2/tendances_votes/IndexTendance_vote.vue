<template>
  <div class="header-contentys pt-3 d-flex align-items-center">
    <div class="mr-5">
      <h4 class="text-app font-weight-bold">Fiche de tendance de vote</h4>
    </div>
    <div class="btn-group page-nav" role="group">
      <div>
        <router-link class="btn" :to="{ name: 'mobilization' }" v-if="
          parseInt(
      ...
      <div>
        <router-link class="btn" :to="{ name: 'climat' }" :class="{ active: this.$route.name === 'climat' }" v-if="
          parseInt(
            this.getPermissionActionByEntity('mobilization_sheets'),
            10
          ) >= 4
      ...
      <div>
        <router-link class="btn" :to="{ name: 'incident' }" :class="{ active: this.$route.name === 'incident' }" v-if="
          parseInt(
            this.getPermissionActionByEntity('mobilization_sheets'),
            10
          ) >= 4
        " data-bs-toggle="tooltip" data-bs-placement="right" title="incident">
          <i class="pi pi-exclamation-circle" style="color: #3242c5"></i> Alerte
          et Incident
        </router-link>
      </div>
       <div>
      ...
      <div>
        <router-link class="btn" :to="{ name: 'bureaux' }" :class="{ active: this.$route.name === 'bureaux' }" v-if="
          parseInt(
            this.getPermissionActionByEntity('mobilization_sheets'),
            10
          ) >= 4
    ...
    <div class="px-1 d-flex mr-4">
      <button type="button" class="btn-app btn-active" @click="openCreateModal()" v-if="
        [6, 3, 2, 7].includes(
          parseInt(this.getPermissionActionByEntity('mobilization_sheets'))
        )
      ">
...
import CreateFicheTendance_vote from './CreateFicheTendance_vote.vue';
import ViewFIcheTendance_vote from './ViewFIcheTendance_vote.vue';
...
export default {
  ...
  mounted() {
    ...
    const mobilizationAction = this.getPermissionActionByEntity(
      "mobilization_sheets"
    );
  ...
  methods: {
    ...
    getPermissionActionByEntity(entityName) {
      const permission = this.currentUser().permissions.find(
        (permission) => permission.entity === entityName
      );
      return permission ? permission.action : null;
    },
     openModal(objetData) {
       this.$dialog.open(ViewFIcheTendance_vote, {
         props: {
           header: "Fiche de tendance de vote " + objetData.id,
           style: {
             width: '50vw'
           },
           modal: true
     ...
     openCreateModal() {
       this.$dialog.open(CreateFicheTendance_vote, {
         props: {
           header: "Fiche de tendance de vote ",
           style: {
             width: '50vw'
           },
           modal: true
