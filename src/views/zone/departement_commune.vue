  ...
  <v-container>
    <v-row>
      <v-breadcrumbs :items="items" class="mb-4">
        <template v-slot:prepend>
          ...
          <v-card-title class="text-primary">Départements /Communes</v-card-title>
        </template>
    ...
    <v-container class="mt-8">
      ...
      <v-card class="mt-8">
        <v-spacer></v-spacer>
        <v-card-title>
          Tableau des Déparetements/Communes
          <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    { key: "code", title: "Code" },
      ...
      { key: "province_code", title: "Code province" },
    ...
    getAllDepartement() {
      ...
      if (accessToken != null) {
        ...
        this.$axios
          ...
          .then((response) => {
            console.log("com_dep =", response.data);
    ...
    handleFileChange(event) {
       ...
       readFile(file) {
         ...
         reader.onload = async (e) => {
           ...
           jsonData[0][4] = "province_code"


           console.log(jsonData[0])
           const json = jsonData.map((element) => {
             const [code, libelle,total_registered,total_offices, province_code] = element;
             return { code, libelle,total_registered,total_offices, province_code};
           });
           ...
           this.$axios
          .post("com_dep/add_all", json).then((response) => {
